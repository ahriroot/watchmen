...
pub async fn pause(args: FlagArgs, config: Config) -> Result<(), Box<dyn Error>> {
    let taskflags = taskflag_to_request(args, config.clone()).await?;
    if taskflags.is_empty() {
        print_result(vec![Response::wrong("No task to pause".to_string())]).await;
    } else {
        let mut requests = Vec::new();
        for taskflag in taskflags {
            requests.push(Request {
                command: Command::Pause(taskflag),
            });
        }
